...
var _ persistence.PersisterNSQL = new(PersisterNSQL)
var _ storage.Transactional = new(PersisterNSQL)
...
func (p *PersisterNSQL) BeginTX(ctx context.Context) (_ context.Context, err error) {
...
func (p *PersisterNSQL) Commit(ctx context.Context) (err error) {
...
func (p *PersisterNSQL) Rollback(ctx context.Context) (err error) {
...
func NewPersister(ctx context.Context, c *pop.Connection, r Dependencies, config *config.DefaultProvider, extraMigrations []fs.FS, goMigrations []popx.Migration) (*PersisterNSQL, error) {
...
func (p *PersisterNSQL) DetermineNetwork(ctx context.Context) (*networkx.Network, error) {
	return p.p.Determine(ctx)
}

func (p PersisterNSQL) WithFallbackNetworkID(nid uuid.UUID) persistence.PersisterNSQL {
	p.fallbackNID = nid
	return &p
}

func (p *PersisterNSQL) CreateWithNetwork(ctx context.Context, v interface{}) error {
	n := p.NetworkID(ctx)
	return p.Connection(ctx).Create(p.mustSetNetwork(n, v))
}

func (p *PersisterNSQL) UpdateWithNetwork(ctx context.Context, v interface{}) (int64, error) {
	n := p.NetworkID(ctx)
...
func (p *PersisterNSQL) NetworkID(ctx context.Context) uuid.UUID {
	return p.r.Contextualizer().Network(ctx, p.fallbackNID)
}

func (p *PersisterNSQL) QueryWithNetwork(ctx context.Context) *pop.Query {
	return p.Connection(ctx).Where("nid = ?", p.NetworkID(ctx))
}

func (p *PersisterNSQL) Connection(ctx context.Context) *pop.Connection {
	return popx.GetConnection(ctx, p.conn)
}

func (p *PersisterNSQL) Ping() error {
	type pinger interface{ Ping() error }
	return p.conn.Store.(pinger).Ping()
}

func (p *PersisterNSQL) mustSetNetwork(nid uuid.UUID, v interface{}) interface{} {
...
func (p *PersisterNSQL) Transaction(ctx context.Context, f func(ctx context.Context, c *pop.Connection) error) error {
